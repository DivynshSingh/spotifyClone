...
async function main(){
    ...
    for(let i=0; i<songs.length; i++){
        ...
        let meta;
        await new Promise((res,rej)=>{jsmediatags.read(src, {
            onSuccess: function(tag){meta=tag; 
                // console.log(tag); 
                res();},
...
main();
main();
// function NameFromHref(url) {
//     let filename = url.split('/').pop(); // Get the last part after splitting by '/'
//     filename = decodeURIComponent(filename); // Decode URL-encoded characters
//     filename = filename.replace('.mp3', ''); // Remove file extension (assuming .mp3)//   /\.[^/.]+$/
//     filename = filename.replace('-', ' ').replace('_', ' '); // Replace '-' and '_' with spaces//    /[-_]/g
//     return filename;
// }
let playMini=async(address)=>{
    // console.log(a.children[1]);
    // console.log(a.children[1].children[0].innerText);
    // console.log(a.children[1].children[1].innerText);
    // let meta=await readMP3Tags(src);
    // let meta;
    ...
    currSong.addEventListener('loadedmetadata', ()=>{
        ...
        if(document.querySelector('.player-par').children[0]){
            ...
            document.getElementsByClassName('playin-info')[0].children[0].innerText=a.children[1].children[0].innerText;
            document.getElementsByClassName('playin-info')[0].children[1].innerText=a.children[1].children[1].innerText;
            return res();
        }
